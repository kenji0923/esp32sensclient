...
static RTC_DATA_ATTR uint8_t server_mac[6];
...
static void save_config() {
  ...
  if (nvs_open("storage", NVS_READWRITE, &h) == ESP_OK) {
    nvs_set_blob(h, "cfg", &active_config, sizeof(active_config));
    nvs_set_blob(h, "smac", server_mac, 6);
...
static void load_config() {
  ...
  if (nvs_open("storage", NVS_READONLY, &h) == ESP_OK) {
    size_t sz = sizeof(active_config);
    if (nvs_get_blob(h, "cfg", &active_config, &sz) != ESP_OK || sz != sizeof(active_config)) {
      memset(&active_config, 0, sizeof(active_config));
    }
    sz = 6; nvs_get_blob(h, "smac", server_mac, &sz);
    nvs_close(h);
  }
}
...
static void esp_now_recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len) {
  ...
  if (current_state == STATE_PING_ACCEPTING && type == MSG_SERVER_HELLO) {
    memcpy(server_mac, recv_info->src_addr, 6);
...
static void read_sensors(sensor_data_t *out) {
  ...
  uint32_t sht_wait_ms = (active_config.sht_read_wait_time_ms > 0) ? active_config.sht_read_wait_time_ms : 30;
  int s_max_trials = s_avg * 3;
  int s_trials = 0;
  for (int trial = 0; trial < s_max_trials && t_count < s_avg; trial++) {
    s_trials++;
    uint8_t cmd = 0xFD; uint8_t d[6];
    if (i2c_master_write_to_device(I2C_PORT_NUM, SHT40_ADDR, &cmd, 1, 100) == ESP_OK) {
      vTaskDelay(pdMS_TO_TICKS(sht_wait_ms));
  ...
  out->temphumid_validcount = (uint16_t)t_count;
  out->temphumid_trial = (uint16_t)s_trials;
  ...
  int d_max_trials = d_avg * 3;
  int d_trials = 0;
  uint32_t dps_wait_ms = (active_config.dps_read_wait_time_ms > 0) ? active_config.dps_read_wait_time_ms : 120;
  ...
  if (i2c_master_write_read_device(I2C_PORT_NUM, DPS310_ADDR, &c_addr, 1, regs, 18, 100) == ESP_OK) {
    ...
    for (int trial = 0; trial < d_max_trials && p_cnt < d_avg; trial++) {
      d_trials++;
      i2c_master_write_to_device(I2C_PORT_NUM, DPS310_ADDR, (uint8_t[]){0x08, 0x02}, 2, 100); vTaskDelay(pdMS_TO_TICKS(dps_wait_ms));
      i2c_master_write_to_device(I2C_PORT_NUM, DPS310_ADDR, (uint8_t[]){0x08, 0x01}, 2, 100); vTaskDelay(pdMS_TO_TICKS(dps_wait_ms));
  ...
  out->pressure_validcount = (uint16_t)p_cnt;
  out->pressure_trial = (uint16_t)d_trials;
...
void app_main(void) {
  ...
  for (;;) {
    if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_UNDEFINED) {
      if (active_config.config_hash == 0) current_state = STATE_PING_ACCEPTING;
      else current_state = STATE_READ;
      wake_count = 0;
    }
    ...
    if (current_state == STATE_CONFIG) {
      add_peer(server_mac);
      uint8_t msg = MSG_CLIENT_READY; esp_now_send(server_mac, &msg, 1);
      if (xSemaphoreTake(msg_sem, pdMS_TO_TICKS(10000)) == pdTRUE) {
        save_config(); current_state = STATE_READ; wake_count = 0;
      } else current_state = STATE_PING_ACCEPTING;
    }

    if (current_state == STATE_READ) {
      wake_count++; i2c_bus_reset();
      ...
      if (i2c_param_config(I2C_PORT_NUM, &conf) == ESP_OK && i2c_driver_install(I2C_PORT_NUM, I2C_MODE_MASTER, 0, 0, 0) == ESP_OK) {
        uint32_t work_delay_ms = (active_config.work_delay_ms > 0) ? active_config.work_delay_ms : 50;
        vTaskDelay(pdMS_TO_TICKS(work_delay_ms));
        sensor_data_t data = {.config_hash = active_config.config_hash, .timestamp = wake_count};
        read_sensors(&data);
        uint8_t buf[sizeof(sensor_data_t) + 1]; buf[0] = MSG_CLIENT_DATA; memcpy(buf + 1, &data, sizeof(sensor_data_t));
        add_peer(server_mac); esp_now_send(server_mac, buf, sizeof(buf));
        if (xSemaphoreTake(msg_sem, pdMS_TO_TICKS(1000)) == pdTRUE && reconfig_requested) {
